...
export const useProdutoStore = defineStore("produtos", () => {
  ...
  const getProdutos = async (categoria = null) => {
    state.loading = true;
    try {
      const response = await ProdutosService.getProdutos(categoria);
      state.produtos = response.data.results || response.data;
    } catch (error) {
      state.error = error;
      console.error(error);
    } finally {
  ...
  const getProduto = async (id) => {
    ...
    try {
      ...
      state.selectedProduto = response.data; // ✅ define o produto atual
      return response;
    } catch (error) {
      state.error = error;
      console.error(error);
